...
def hybrid_a_star(
    ...
    on_step: Callable[[tuple[float, float, float], set[tuple[int, int, int]]],
                      None] | None = None,
    ...
    Args:
        ...
        on_step: 매 expand 직후 호출되는 viz 콜백 — `(current_pose, frontier_keys)`.
    ...
    #   4. on_step(pose_cur, set(open_dict.keys())) 호출 (있으면)
    ...
    #      - arc_collision(pose_cur, yaw_rate, dt, vx, obstacles) 면 skip
