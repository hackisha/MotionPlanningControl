"""RRT — Rapidly-exploring Random Tree (sampling-based path planner).
...
def rrt(
    ...
    Args:
        ...
        on_step: 매 새 node 추가 직후 호출 — `(new_node, parent_node, iteration)`.
    ...
    # 힌트:
    #   - nodes: list[(x, y)] — tree 의 node. nodes[0] = start.
    #   - parents: dict[node_index, parent_index_or_None] — parent chain.
    #   - rng = random.Random(seed) — RNG 인스턴스 (전역 random 오염 방지).
    #
    ...
    #   2. Nearest: nodes 중 sample 과 가장 가까운 node 의 index — `_euclid` 사용.
    #   3. Steer: `_steer(nearest, sample, eta)` → new_pt (eta 거리로 잘림).
    #      만약 new_pt 가 nearest 와 거의 같으면 (`_euclid < 1e-6`) 이 iter 건너뜀.
    #   4. Collision-free: `_is_collision_free(nearest, new_pt, obstacles)` 가 False 면 skip.
    #   5. Add to tree:
    #        new_i = len(nodes); nodes.append(new_pt); parents[new_i] = nearest_i
    #        on_step(new_pt, nearest, it) 호출 (있으면)
    #   6. Goal check: `_euclid(new_pt, goal) < goal_range` 면 path 복원:
    #        parents[new_i] → parents[그 parent] → ... → None 까지 nodes 누적, 뒤집어 반환.
    #
    # max_iter 다 돌면 [] 반환.
    #
