"""A* regression — behavioral spec (requirements level).

알고리즘 형태 (정통 A* / heapq priority queue / 다른 변종) 는 제약 X.
인터페이스 + 경로의 도달성·8-connectivity·최단성으로 합격 판정.
weight_heuristic=1.0 (admissible) 에서 최적 cost 검증.
"""
import math

from a_star import a_star
from map_astar import GOAL, OBSTACLES, START

# Reference (weight=1.0) 실측: 약 70.28. 1.05× 까지 허용 → tie-breaking 차이 수용.
_OPTIMAL_COST_BOUND = 74.0
...
def test_path_starts_and_ends_correctly():
    path = a_star(START, GOAL, OBSTACLES, weight_heuristic=1.0)
...
def test_path_near_optimal_with_admissible_heuristic():
    """weight=1.0 일 때 cost 가 optimal bound 이내 (admissible heuristic)."""
    path = a_star(START, GOAL, OBSTACLES, weight_heuristic=1.0)
    cost = _path_cost(path)
    assert cost < _OPTIMAL_COST_BOUND, (
        f"path cost {cost:.2f} 가 임계값 {_OPTIMAL_COST_BOUND} 초과 — "
        f"admissible A* 가 최단 경로를 찾지 못함")


def test_on_step_callback_invoked():
    """on_step 콜백이 expand 횟수만큼 호출됨 (viz 인터페이스 sanity)."""
    calls: list[tuple[tuple[int, int], int]] = []

    def cb(current: tuple[int, int], frontier: set[tuple[int, int]]) -> None:
        calls.append((current, len(frontier)))

    path = a_star(START, GOAL, OBSTACLES, weight_heuristic=1.0, on_step=cb)
